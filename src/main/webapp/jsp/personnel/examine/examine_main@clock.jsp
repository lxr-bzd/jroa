...
<title>查询列表</title>
<script>
var toAddUrl = '${path}/personnel/examine/examine/toedit.do?sysModule=colck';
var deleteUrl = '${path}/personnel/examine/examine/delete.do';
var toEditUrl = '${path}/personnel/examine/examine/toedit.do';
var toInfoUrl = '${path}/personnel/apply/apply/toinfo.do?sysModule=clock';
var updateUrl = '${path}/personnel/examine/examine/update.do';
    ...
    function editById(id){
    	$app.dialog(toEditUrl+'?sysModule=colck&sysAction=edit&id='+id+"",function(){
			refleshData('mainTable');
...
function operatorFormatter(value, row, index) {
	...
	switch (row.state) {
	case 1:
		operator+= $app.btn({type:'btn-warning',img:'glyphicon-ok'},'审批','editById(\''+row.id+'\')');
...
<body class="mlr15">
    ...
    <div class="rightinfo explain_col">
    	...
    	<table class="table_list" id="mainTable" data-toggle="table"
			data-url="${path}/personnel/examine/examine/view.do?applytype=5" data-pagination="ture" 
			...
			<thead>
				<tr>
					...
					<th data-field="starttime"  data-formatter="$app.tableUi.time">未打卡时间</th>
